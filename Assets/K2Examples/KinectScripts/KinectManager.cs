...
//	public bool useOwnThread = false; 
    ...
    // List of all users
    [SerializeField] protected List<Int64> alUserIds = new List<Int64>();
...
//					}
    ...
    private void UpdateKinectStreams()
    {
        if (kinectInitialized)
        {
            ...
            if (computeUserMap != UserMapType.None)
            {
                sensorData.firstUserIndex = liPrimaryUserId != 0 && dictUserIdToIndex.ContainsKey(liPrimaryUserId) ? dictUserIdToIndex[liPrimaryUserId] : -1;
...
//						}
                        ...
                        {
                            if (listener != null && listener.GestureCompleted(userId, userIndex, gestureData.gesture, (KinectInterop.JointType)gestureData.joint,
                                    gestureData.screenPos))
                            {
                                ResetPlayerGestures(userId);
                            }
                        }
...
//						else
                ...
                if (joint >= 0)
                {
                        ...
                        IsJointTracked(UserId, (int)KinectInterop.JointType.ShoulderLeft) && IsJointTracked(UserId, (int)KinectInterop.JointType.ShoulderRight))
                    {
                        ...
                        playerJointsPos[joint] = (GetJointPosition(UserId, (int)KinectInterop.JointType.ShoulderLeft) +
                                                  GetJointPosition(UserId, (int)KinectInterop.JointType.ShoulderRight)) / 2f;
                    }
                }
            }
    ...
    protected virtual bool CheckForCalibrationPose(Int64 UserId, int bodyIndex, KinectGestures.Gestures calibrationGesture)
    {
        ...
        KinectGestures.GestureData gestureData = playerCalibrationData.ContainsKey(UserId) ? playerCalibrationData[UserId] : new KinectGestures.GestureData();
