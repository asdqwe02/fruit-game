...
public class InteractionManager : MonoBehaviour
{
    ...
    [SerializeField] private Dictionary<Int64, Vector3> leftHandPosDict = new Dictionary<Int64, Vector3>();
    [SerializeField] private Dictionary<Int64, Vector3> leftHandScreenPosDict = new Dictionary<Int64, Vector3>();
    ...
    [SerializeField] private Dictionary<Int64, Vector3> rightHandPosDict = new Dictionary<Int64, Vector3>();

    [SerializeField] private Dictionary<Int64, Vector3> rightHandScreenPosDict = new Dictionary<Int64, Vector3>();
    ...
    {
        return leftHandScreenPosDict[userID];
    }
    ...
    {
        return rightHandScreenPosDict[userId];
    }
    ...
    public UnityEvent<Int64, bool> UserIdChange;
    ...
    void Update()
    {
        ...
        if (kinectManager && kinectManager.IsInitialized())
        {
            ...
            if (playerUserIDList.Count > 0)
            {
                ...
                foreach (KeyValuePair<Int64, Vector3> leftHandPos in leftHandPosDict)
                {
                    if (playerUserIDList.Contains(leftHandPos.Key))
                    {
                        Int64 key = leftHandPos.Key;
                        ...
                        isleftIboxValid = kinectManager.GetLeftHandInteractionBox(
                            ...
                            ref leftIboxRightTopFrontTemp, isleftIboxValid
                        );
                    ...
                    {
                        if (!removeId.Contains(leftHandPos.Key))
                        {
                            removeId.Add(leftHandPos.Key);
                        }
                ...
                foreach (var rightHandPos in rightHandPosDict.Keys)
                {
                    if (playerUserIDList.Contains(rightHandPos))
                    {
                        Int64 key = rightHandPos;
                    ...
                    {
                        if (!removeId.Contains(rightHandPos))
                        {
                            removeId.Add(rightHandPos);
                        }
                    }
                }
            ...
            if (playerUserID != 0)
            {
                    ...
                    kinectManager.GetJointTrackingState(playerUserID, (int)KinectInterop.JointType.HandLeft) != KinectInterop.TrackingState.NotTracked)
                {
                    ...
                    leftHandPressProgress = (Time.realtimeSinceStartup - lastLeftHandPressTime) >= KinectInterop.Constants.ClickStayDuration &&
                                            leftHandScreenPos.z >= 0.7f
                        ? (leftHandScreenPos.z - 0.7f) / 0.3f
                        : 0f;
                         ...
                         (allowPushToClick && leftHandPressProgress > 0f)))
                    {
                        ...
                        else
                        {
                            ...
                            float leftHandTimeProgress =
                                allowHandClicks && (Time.realtimeSinceStartup - lastLeftHandClickTime) >= (KinectInterop.Constants.ClickStayDuration / 2f)
                                    ? ((Time.realtimeSinceStartup - lastLeftHandClickTime - (KinectInterop.Constants.ClickStayDuration / 2f)) * 2f /
                                       KinectInterop.Constants.ClickStayDuration)
                                    : 0f;
                    ...
                    kinectManager.GetJointTrackingState(playerUserID, (int)KinectInterop.JointType.HandRight) != KinectInterop.TrackingState.NotTracked)
                {
                    ...
                    rightHandPressProgress = (Time.realtimeSinceStartup - lastRightHandPressTime) >= KinectInterop.Constants.ClickStayDuration &&
                                             rightHandScreenPos.z >= 0.7f
                        ? (rightHandScreenPos.z - 0.7f) / 0.3f
                        : 0f;
                         ...
                         (allowPushToClick && rightHandPressProgress > 0f)))
                    {
                        ...
                        {
                            // show progress after the 1st half of the needed duration
                            float rightHandTimeProgress =
                                allowHandClicks && (Time.realtimeSinceStartup - lastRightHandClickTime) >= (KinectInterop.Constants.ClickStayDuration / 2f)
                                    ? ((Time.realtimeSinceStartup - lastRightHandClickTime - (KinectInterop.Constants.ClickStayDuration / 2f)) * 2f /
                                       KinectInterop.Constants.ClickStayDuration)
                                    : 0f;
